  ...
  return (
    <>
      <Grid container direction="row">
        ...
        >
          <Typography variant="subtitle2" fontSize="medium" style={{ fontWeight: 'bold' }}> 300 notes.</Typography>
        </Grid>
        <Grid item xs={2} sx={{ pr: 0.5 }}>
          <IconButton aria-label="back notes">
            <Typography variant="subtitle2" style={{ fontWeight: 'bold', color: 'black', fontSize: 28 }}>#</Typography>
          </IconButton>
        </Grid>
      </Grid>
      <Divider />
      <Stack direction="row" spacing={2} sx={{ pt: 1, pl: 1, pb: 0.5 }} style={{ overflow: 'hidden' }}>
        {
            likes.map((e) => (
              <Badge
                key={e.id}
              ...
              >
                <Avatar src={e.blog_avatar} sx={{ width: 24, height: 24 }} alt="Travis Howard" />
              </Badge>
            ))
        }
        {
