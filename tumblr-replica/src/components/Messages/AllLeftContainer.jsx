import './css/leftContainer.css';
import React from 'react';
import { useSelector } from 'react-redux';
import Avatar from '@mui/material/Avatar';
import Grid from '@mui/material/Grid';
import { useMediaQuery } from 'react-responsive';
import { getAllMessages } from '../../states/retriveallmessagesslice/retriveallmessagesslice';
import PostCard from '../AskSubmitPosts/subcomponents/PostCard/PostCard';

function AllLeftContainer() {
  const Posts = useSelector(getAllMessages).response.posts;
  const isDesktopOrLaptop = useMediaQuery({
    query: '(min-width: 1224px)',
  });
  return (
    <div className="lContainer">
      {Posts
        && Posts.map((post) => (

          <Grid
            item
            xs
            container
            direction="row"
            key={post.post_type === 'submit' ? post.post_id : post.question_id}
            spacing={2}
            style={{ justifyContent: 'center', alignItems: 'flex-start', display: 'flex' }}
            sx={{ mb: 2, mt: 0 }}
          >
            {isDesktopOrLaptop && (
              <Grid item>
                <Avatar
                  // eslint-disable-next-line no-nested-ternary
                  src={post.post_type === 'submit' ? post.blog_avatar : post.question_flag ? 'data:image/png;base64,' : post.blog_avatar}
                  variant="square"
                  xs={2}
                  style={{
                    maxWidth: 64,
                    minWidth: 64,
                    maxHeight: 64,
                    minHeight: 64,
                  }}
                />
              </Grid>
            )}
            <Grid item>
              <PostCard
                postId={post.post_type === 'submit' ? post.post_id : post.question_id}
                postTime={post.post_type === 'submit' ? post.post_date : post.ask_time}
                blogId={post.post_type === 'submit' ? post.blog_id : post.blog_id}
                // eslint-disable-next-line no-nested-ternary
                blogUsername={post.post_type === 'submit' ? post.blog_username : post.question_flag ? 'Anonymous' : post.blog_username}
                postBody={post.post_type === 'submit' ? post.post_body : post.question_body}
                // eslint-disable-next-line no-nested-ternary
                blogAvatar={post.post_type === 'submit' ? post.blog_avatar : post.question_flag ? 'data:image/png;base64,' : post.blog_avatar}
                postType={post.post_type}
                xs={10}
                sx={{ mt: 0 }}
              />
            </Grid>
          </Grid>

        ))}
    </div>
  );
}

export default AllLeftContainer;
