...
const FollowAsynch = createAsyncThunk(
  'follow_blog/:blog_id',
  async (BlogId) => {
    console.log(BlogId, 'hi omer');
    console.log(AuthStr);
    const response = await FollowApi.post('follow_blog/2', { headers: { Authorization: AuthStr, Accept: 'application/json', 'Content-Type': 'application/json' } });
    console.log(response.data, 'hola hola hola');
    console.log(BlogId, 'hi abdehamed');
