    ...
    chats.map((chat) => (
      <div key={chat.id}>
        <ListItemButton
          ...
          onClick={() => {
            const newavatars = avatars.filter((el) => chat.id === el.elem.friend_id);
            console.log(avatars);
            if (newavatars.length) {
              dispatch(removeAvaterID(newavatars[0]));
            } else
            if ((chatBox.length
                      && chatBox[0].elem.friend_id !== chat.id) || chatBox.length === 0) {
              dispatch(getChatRoomId({
                blogsID: {
                  from_blog_id: User.primaryBlogId,
                  to_blog_id: newMessagePress1 ? chat.id : chat.friend_id,
                },
                User,
                elem: chat,
              }));
        ...
        >
          <ListItemAvatar>
            <Avatar src={newMessagePress1 ? chat.avatar : chat.friend_avatar} style={{ margin: '5px' }} />
          </ListItemAvatar>
          {
            newMessagePress1 ? (
              <Box>
...
SearchList.propTypes = {
  chats: PropTypes.arrayOf(Ob).isRequired,
};
