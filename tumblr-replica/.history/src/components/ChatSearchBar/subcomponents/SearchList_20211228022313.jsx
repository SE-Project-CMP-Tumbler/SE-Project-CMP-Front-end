    ...
    chats.map((chat) => (
      <div key={chat.blog_id}>
        <ListItemButton
          ...
          onClick={() => {
            const newavatars = avatars.filter((el) => chat.blog_id === el.elem.friend_id);
            console.log(avatars);
            if (newavatars.length) {
              dispatch(removeAvaterID(newavatars[0]));
            } else
            if ((chatBox.length
                      && chatBox[0].elem.friend_id !== chat.blog_id) || chatBox.length === 0) {
              dispatch(getChatRoomId({
                blogsID: {
                  from_blog_id: User.primaryBlogId,
                  to_blog_id: newMessagePress1 ? chat.blog_id : chat.friend_id,
                },
                User,
                elem: chat,
              }));
