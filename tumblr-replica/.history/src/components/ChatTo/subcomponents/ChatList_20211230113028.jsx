    ...
    chats.map((chat) => (
      <div key={chat.friend_id}>
        ...
        >
          ...
          {
            newMessagePress1 ? (
              <Link to={`/messaging/conversation/${User.primaryBlogId}/${chat.friend_username}`} className="link">
                <Box>
              ...
              <Link to={`/messaging/conversation/${User.primaryBlogId}/${chat.friend_username}`} className="link"></Link>
              n<Box>
              n  <p className="chat-par-name">
              n    {chat.friend_username}
              n  </p>
              n  {chat.text ? (
              n    <p className="lastmessage">
              n      {`${chat.blog_username}:${chat.text}`}
              n    </p>
              n  ) : null}
              n  {chat.photo ? (
              n    <p className="lastmessage">
              n      {`${chat.blog_username}:sent a post`}
              n    </p>
              n  ) : null}
              n  {chat.gif ? (
              n    <p className="lastmessage">
