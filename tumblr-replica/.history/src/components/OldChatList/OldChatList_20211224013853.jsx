...
function OldChatList() {
  ...
  const chat_room_id = useSelector((state) => state.Chat.chatroomid);
    ...
    <Card sx={{ maxWidth: 260 }}>
          ...
          <Grid container spacing={5}>
            ...
            <Grid item>
              <button
                ...
                style={{
                  marginLeft: '1px', color: 'rgb(13, 131, 160)', backgroundColor: 'transparent', border: 'none', cursor: 'pointer',
                }}
              >
                New Message
              </button>
      ...
      <CardContent align="center">
        ...
        <Grid container spacing={1}>
          {myFriends.map((elem) => (
            <Grid item xs={2.6} key={elem.blog_id}>
              <button
                type="button"
                onClick={() => {
                  ...
                  dispatch(setChatBoxesID({
                    chatRoomId: elem.blog_id
                    id: elem.blog_id,
                    img: elem.blog_avatar,
                    friendName: elem.blog_username,
                  }));
                }}
                style={{ backgroundColor: 'transparent', border: 'none', cursor: 'pointer' }}
              >
                <img className="chat-friend-img" alt="not Fn" src={elem.blog_avatar} />
              </button>
            </Grid>
          ))}
        </Grid>
