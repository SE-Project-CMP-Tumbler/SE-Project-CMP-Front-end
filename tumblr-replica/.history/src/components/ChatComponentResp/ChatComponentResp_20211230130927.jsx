...
function ChatComponentResp({ friendName, id, img }) {
  ...
  useEffect(() => {
    dispatch(getBlogIdFromBlogUN({ blogId: params.username, User})).then((id1) => {
      dispatch(getBlogIdFromBlogUN({blogId: params.username, User})).then((id2) => {
        dispatch(getChatRoomIdRes({blogsID: {from_blog_id:id1, to_blog_id:id2 }, User })).then((chat_room_id) => {
          dispatch(getChatFeed({chatRoomId:chat_room_id, User}))
        })
