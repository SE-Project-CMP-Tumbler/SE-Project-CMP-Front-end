...
function ChatComponentResp({ friendName, id, img }) {
  ...
  useEffect(() => {
    dispatch(getBlogIdFromBlogUN({ blogId: params.username, User, isUser }))
      .then((id1) => {
        dispatch(getBlogIdFromBlogUN({ blogId: params.friendname, User }))
          .then((id2) => {
  ...
  return (
    ...
    >
      <ChatTopBarResp friendName={params.friendname} id={id} img={img} />
      <ChatFeed img={img} friendName={params.friendname} />
      <ChatInputResp id={id} />
    </Box>
  );
}
