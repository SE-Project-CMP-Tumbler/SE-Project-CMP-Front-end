...
function ChatComponentResp() {
  ...
  useEffect(() => {
    ...
    dispatch(
      getBlogIdFromBlogUN({ blogUserName: params.username, User, isUser: true })
    ).then((id1) => {
      console.log(id1.payload);
      dispatch(
        getBlogIdFromBlogUN({
          blogUserName: params.friendname,
          User,
          isUser: false,
        })
      ).then((id2) => {
        ...
        dispatch(
          getChatRoomIdRes({
            blogsID: { from_blog_id: id1.payload, to_blog_id: id2.payload },
            User,
          })
        ).then((chatRoom) => {
          console.log(chatRoom);
          setChatRoomId(chatRoom.payload);
          dispatch(getChatFeed({ chatRoomId: chatRoom.payload, User })).then(
            () => {
              console.log("finaaal");
              dispatch(
                setBlogFriendName({ blogUserName: params.friendname, User })
              );
            }
          );
        });
  ...
  return (
    <Box className={classes.chatbox}>
      <ChatTopBarResp friendName={params.friendname} id={userbloginfo.id} />
      <ChatFeed
        showImg={showImg}
