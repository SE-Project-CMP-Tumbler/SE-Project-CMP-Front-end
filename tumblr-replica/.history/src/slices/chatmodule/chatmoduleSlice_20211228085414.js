  ...
  reducers: {
    ...
    filterBlogsChat: (state, { payload }) => {
      ...
      const newFilter = newstate.blogsToChat.filter(
        (value) => value.username.toLowerCase().includes(payload.toLowerCase()),
      );
  ...
  extraReducers: {
    ...
    [sendMessage.fulfilled]: (state, { payload }) => {
      ...
      if (SERVICETYPE === MOCK) {
        ...
        {
          const newmessage = {...messageBody, blog_id: payload?.User.primaryBlogId,
          blog_username: payload?.User.primaryBlogId",
