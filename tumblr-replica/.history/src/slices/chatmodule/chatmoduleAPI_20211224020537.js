...
export const sendMessage = createAsyncThunk('Chat/sendMessage', async ({
  ...
  try {
    if (SERVICETYPE === MOCK) {
      const response = await api.patch(`/mockmessages/${id}`, { chat_messages: newmessages });
      console.log('hi frim fetch follower');
      console.log(response.data);
...
export const deleteMessages = createAsyncThunk('Chat/deleteMessages', async (chatRoomId) => {
  try {
    if (SERVICETYPE === MOCK) {
    ...
    console.log(response.data);
    return response.data;
    const response = await apiR.delete(`/clear_chat/${chatRoomId}`);
    return response.data;
  } catch (err) {
    throw Error(err);
  }
