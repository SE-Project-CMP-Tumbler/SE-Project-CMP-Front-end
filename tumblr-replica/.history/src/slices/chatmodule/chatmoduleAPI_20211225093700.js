...
export const sendMessage = createAsyncThunk('Chat/sendMessage', async ({
  ...
  try {
    if (SERVICETYPE === MOCK) {
      const response = await api.patch(`/messages/${chatRoomId}`, { responsechat_messages: newmessages });
...
export const getChatRoomId = createAsyncThunk('Chat/sendMessage', async ({
  blogsID, User, elem,
}) => {
  try {
    if (SERVICETYPE === MOCK) {
