...
const chatReducer = createSlice({
  ...
  reducers: {
    ...
    removeAvaterID: (state, { payload }) => {
      const newstate = state;
      console.log(payload?.res.response.chat_room_id);
      const newAvaters = state.avatars.filter((elem) => {
        console.log(elem);
        return elem.res.response.chat_room_id !== payload?.res.response.chat_room_id;
      });
