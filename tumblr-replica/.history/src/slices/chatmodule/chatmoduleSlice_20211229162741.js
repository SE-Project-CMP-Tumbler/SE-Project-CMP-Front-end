...
import {
  getChatFeed, getAllChats, searchForChat,
...
const chatReducer = createSlice({
  ...
  reducers: {
    ...
    chatUpdate: (state, { payload }) => {
      const newstate = state;
      const pay = {
        text: payload.text, photo:payload.photo, gif: payload.gif, read: payload.read, blog_username: payload.from_blog_username, from_blog_id: payload.from_blog_username, from_blog_avatar: 'https://cdnb.artstation.com/p/assets/images/images/033/268/113/large/edmerc-d-mercadal-eren-v5.jpg?1609001111', from_blog_avatar_shape: 'circle', from_blog_title: 'nadeen',
      };
  ...
  extraReducers: {
    ...
    [sendMessage.fulfilled]: (state, { payload }) => {
      ...
      if (SERVICETYPE === MOCK) {
        ...
        const newstate = state;
        if (payload?.res.meta === '200') {
          const newmessage = {
            ...payload?.messageBody,
            blog_id: payload?.User.primaryBlogId,
            blog_username: payload?.User.blogName,
            blog_avatar: payload?.User.primaryBlogAvatar,
          };
          console.log(newmessage);
          newstate.chatfeed.push(newmessage);
          console.log('chat feed is got suc ');
        }
      }
    },
